...
let
  mlPython = pkgs.python313.withPackages (
    ps: with ps; [
      numpy
      scipy
      sounddevice
    ...
    text = ''
            ...
            function show_help() {
                ...
                echo -e "    ''${C_WHITE}routine''${NC}   ''${C_MUTED}❯''${NC} Launch Silicon Routine & Self-Improvement Dashboard"
                        ...
                        echo -e "    ''${C_WHITE}image''${NC}       ''${C_MUTED}❯''${NC} Generate high-fidelity images using Gemini Imagen 3"
                    ...
                    case $1 in
                      ...
                      screensaver)
                        ...
                        case "''${1:-}" in
                          ...
                          generate)
                            PROMPT="''${2:-}"
                            if [ -z "''$PROMPT" ]; then error "Please provide a prompt. Usage: manx screensaver generate <prompt>"; fi
                            log "Generating custom screensaver branding image with Gemini..."
                            manx image "''$PROMPT" "''$BRAND_DIR/logo.png"
                            if [ -f "''$BRAND_DIR/logo.png" ]; then
                                rm -f "''$BRAND_DIR/screensaver.txt" 2>/dev/null || true
                                success "Screensaver branding image generated. Launching preview..."
                                pkill -f 'alacritty --class manx-screensaver' || true
                                "$HOME/.local/bin/manx-screensaver" --force
                            else
                                error "Failed to generate screensaver branding image."
                            fi
                            ;;
                          toggle)
                          ...
                          *)
                            ...
                            echo -e "    ''${C_WHITE}generate''${NC}   ''${C_MUTED}❯''${NC} Generate a branding image using Gemini AI"
                      ...
                      image|image-gen)
                        shift
                        PROMPT="''${1:-}"
                        if [ -z "''$PROMPT" ]; then error "Please provide a prompt. Usage: manx image <prompt> [output_path]"; fi
                        OUT_PATH="''${2:-''$HOME/Downloads/generated_image.png}"
                        GEMINI_TOKEN=""
                        if [ -f "''$HOME/.config/manx/gemini_token" ]; then
                            GEMINI_TOKEN=$(cat "''$HOME/.config/manx/gemini_token")
                        elif [ -n "''${GOOGLE_API_KEY:-}" ]; then
                            GEMINI_TOKEN="''$GOOGLE_API_KEY"
                        elif [ -n "''${GEMINI_API_KEY:-}" ]; then
                            GEMINI_TOKEN="''$GEMINI_API_KEY"
                        fi
                        if [ -z "''$GEMINI_TOKEN" ]; then
                            error "A Gemini API Key is required. Run 'manx agent' or put it in ~/.config/manx/gemini_token"
                        fi
                        log "Generating image with Gemini Imagen 3..."
                        info "Prompt: '''$PROMPT'"
      ...
      try:
          ...
          if "predictions" in data and len(data["predictions"]) > 0:
              img_data_b64 = data["predictions"][0]["bytesBase64Encoded"]
              img_data = base64.b64decode(img_data_b64)
              with open(output_path, "wb") as f:
                  f.write(img_data)
              print("SUCCESS")
          else:
              print(f"ERROR: No predictions found in response: {data}")
              sys.exit(1)
      except Exception as e:
          print(f"ERROR: {e}")
          sys.exit(1)
      ' "''$PROMPT" "''$OUT_PATH" "''$GEMINI_TOKEN"
                        RC=''$?
                        if [ "''$RC" -eq 0 ]; then
                            success "Image generated successfully! Saved to ''$OUT_PATH"
                            if command -v loupe &>/dev/null; then
                                loupe "''$OUT_PATH" &> /dev/null & disown
                            elif command -v xdg-open &>/dev/null; then
                                xdg-open "''$OUT_PATH" &> /dev/null & disown
                            fi
                        else
                ...
                openlane)
                  ...
                  case "''$ACTION" in
                    shell)
                      log "Entering OpenLane 2 Reproducible CAD Workspace..."
                      info "Initializing digital circuit design suite (Yosys, OpenROAD, Magic, Netgen)..."
                      nix shell github:efabless/openlane2
                      ;;
                    test)
                      log "Running OpenLane 2 CAD environment smoke test..."
                      nix shell github:efabless/openlane2 --run "openlane --smoke-test"
                      ;;
                    setup)
                      log "Setting up secure VLSI/ASIC Workstation design directories..."
                      mkdir -p "''$HOME/VLSI"
                      success "VLSI Workspace created at ''$HOME/VLSI."
                      info "To start a project, cd into ~/VLSI and run 'manx openlane shell'"
                      ;;
                    *)
                      echo -e "  ''${C_SECONDARY}Usage:''${NC} ''${C_WHITE}manx openlane''${NC} ''${C_GOLD}<action>''${NC}"
                      echo -e ""
                      echo -e "    ''${C_WHITE}shell''${NC}      ''${C_MUTED}❯''${NC} Enter interactive OpenLane 2 CAD shell"
                      echo -e "    ''${C_WHITE}test''${NC}       ''${C_MUTED}❯''${NC} Run system smoke-test verification"
                      echo -e "    ''${C_WHITE}setup''${NC}      ''${C_MUTED}❯''${NC} Setup personal VLSI project workspace"
                      echo -e ""
                      ;;
                  esac
                  ;;
                ...
                librelane)
                  ...
                  case "''$ACTION" in
                    shell)
                      log "Entering LibreLane Reproducible CAD Workspace..."
                      info "Initializing next-generation ASIC design suite (Yosys, OpenROAD, Magic, Netgen)..."
                      nix shell github:librelane/librelane
                      ;;
                    test)
                      log "Running LibreLane CAD environment smoke test..."
                      nix shell github:librelane/librelane --run "librelane --help"
                      ;;
                    setup)
                      log "Setting up secure VLSI/ASIC Workstation design directories..."
                      mkdir -p "''$HOME/VLSI"
                      success "VLSI Workspace created at ''$HOME/VLSI."
                      info "To start a project, cd into ~/VLSI and run 'manx librelane shell'"
                      ;;
                    *)
                      echo -e "  ''${C_SECONDARY}Usage:''${NC} ''${C_WHITE}manx librelane''${NC} ''${C_GOLD}<action>''${NC}"
                      echo -e ""
                      echo -e "    ''${C_WHITE}shell''${NC}      ''${C_MUTED}❯''${NC} Enter interactive LibreLane CAD shell"
                      echo -e "    ''${C_WHITE}test''${NC}       ''${C_MUTED}❯''${NC} Run system smoke-test verification"
                      echo -e "    ''${C_WHITE}setup''${NC}      ''${C_MUTED}❯''${NC} Setup personal VLSI project workspace"
                      echo -e ""
                      ;;
