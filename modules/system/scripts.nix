  ...
  manx-script = pkgs.writeShellScriptBin "manx" ''
                        ...
                        case $1 in
                          rebuild)
                            ...
                            # Synchronize Open-WebUI API key from secure user file
                            sudo mkdir -p /var/lib/open-webui
                            if [ -f "$HOME/.config/manx/github_token" ]; then
                                log "Synchronizing secure GitHub Models API key with Open-WebUI..."
                                sudo tee /var/lib/open-webui/open-webui.env > /dev/null << EOF
    OPENAI_API_BASE_URL=https://models.github.ai/inference/v1
    OPENAI_API_KEY=$(cat "$HOME/.config/manx/github_token")
    ...
    EOF
                          ...
                          doctor)
                            (
                                ...
                                border_l; echo -ne "  ''${C_PRIMARY}  NixOS Base Environment:''${NC}"; printf "%-44s" ""; border_r
                                ...
                                border_l; printf "%-72s" ""; border_r

                                # Section 2: Core System CLI Packages
                                border_l; echo -ne "  ''${C_PRIMARY}  Core System CLI Packages:''${NC}"; printf "%-42s" ""; border_r
                                border_l; echo -ne "     [''${NH_COL}''${NH_VAL}''${NC}] nh (Nix Helper)   ''${C_MUTED}❯''${NC} "; printf "''${C_WHITE}%-41s''${NC}" "''${NH_INFO}"; border_r
                                border_l; echo -ne "     [''${RG_COL}''${RG_VAL}''${NC}] ripgrep (rg)      ''${C_MUTED}❯''${NC} "; printf "''${C_WHITE}%-41s''${NC}" "''${RG_INFO}"; border_r
                                border_l; echo -ne "     [''${FZF_COL}''${FZF_VAL}''${NC}] fzf finder        ''${C_MUTED}❯''${NC} "; printf "''${C_WHITE}%-41s''${NC}" "''${FZF_INFO}"; border_r
                                border_l; echo -ne "     [''${GIT_COL}''${GIT_VAL}''${NC}] git control        ''${C_MUTED}❯''${NC} "; printf "''${C_WHITE}%-41s''${NC}" "''${GIT_INFO}"; border_r
                                border_l; printf "%-72s" ""; border_r

                                # Section 3: System Services & Keys
                                border_l; echo -ne "  ''${C_PRIMARY}󰌢  System Services & Keys:''${NC}"; printf "%-44s" ""; border_r
