    ...
    case $1 in
     ...
     sudo systemctl restart open-webui || true
      ...
      aider)
        ...
        # Handle Anthropic Models (Elite Cloud)
        elif [[ "$MODEL" == anthropic/* ]]; then
            RAW_MODEL=$(echo "$MODEL" | cut -d'/' -f2)
            TOKEN_FILE="$HOME/.config/manx/anthropic_token"
            if [ -f "$TOKEN_FILE" ]; then
                export ANTHROPIC_API_KEY=$(cat "$TOKEN_FILE")
            elif [ -n "$ANTHROPIC_API_KEY" ]; then
                true
            else
                echo -ne "  ''${C_HIGHLIGHT}❯ Enter your Anthropic API Key:''${NC} "
                read -s -r USER_TOKEN
                echo ""
                if [ -z "$USER_TOKEN" ]; then
            ...
            log "Launching Elite Claude Agent (Aider + $RAW_MODEL)..."
            aider --model "$MODEL" --no-browser --map-tokens 1024 --edit-format whole --watch-files "$@"
      ...
      agent)
        ...
        case $1 in
          update)
            ...
            echo "#!/usr/bin/env bash" > $HOME/.local/bin/interpreter
            echo "export LD_LIBRARY_PATH=\"$LIB_PATH:\$LD_LIBRARY_PATH\"" >> $HOME/.local/bin/interpreter
            echo "exec $HOME/.local/bin/interpreter.real \"\$@\"" >> $HOME/.local/bin/interpreter
          ...
          *)
            ...
            # Handle Anthropic Models (Elite Cloud)
                ...
                log "Launching Elite Claude Agent (Open-Interpreter + $RAW_MODEL)..."
                interpreter --model "$MODEL" "$@"
            # Handle GitHub Models (Free Cloud)
            elif [[ "$MODEL" == github/* ]]; then
                RAW_MODEL=$(echo "$MODEL" | cut -d'/' -f2)
      ...
      doctor)
        (
            ...
            # --- 2. Print Beautiful Borderless Diagnostic Dashboard ---
            
            # Helper function for beautifully aligned diagnostic rows
            print_diag() {
                local icon="$1"
                local color="$2"
                local label="$3"
            ...
            print_diag "''${FLAKES_VAL}" "''${FLAKES_COL}" "Flakes Enablement" "''${FLAKES_INFO}"
            print_diag "''${CHANNELS_VAL}" "''${CHANNELS_COL}" "Declarative State" "''${CHANNELS_INFO}"

            # Section 2: Core System CLI Packages
            echo -e "\n  ''${C_SECONDARY}  Core System CLI Packages''${NC}"
            print_diag "''${NH_VAL}" "''${NH_COL}" "nh (Nix Helper)" "''${NH_INFO}"
            print_diag "''${RG_VAL}" "''${RG_COL}" "ripgrep (rg)" "''${RG_INFO}"
            print_diag "''${FZF_VAL}" "''${FZF_COL}" "fzf finder" "''${FZF_INFO}"
            print_diag "''${GIT_VAL}" "''${GIT_COL}" "git control" "''${GIT_INFO}"

            # Section 3: System Services & Keys
            echo -e "\n  ''${C_HIGHLIGHT}󰌢  System Services & Keys''${NC}"
            print_diag "''${OLLAMA_VAL}" "''${OLLAMA_COL}" "Ollama Service" "''${OLLAMA_INFO}"
            print_diag "''${WEBUI_VAL}" "''${WEBUI_COL}" "Open-WebUI Portal" "''${WEBUI_INFO}"
            print_diag "''${TOKEN_VAL}" "''${TOKEN_COL}" "Secure GH Token" "''${TOKEN_INFO}"
            ...
            if [ "''${ANY_MISSING}" == "true" ]; then
                echo -e ""
                echo -e "  ''${C_GOLD}󰋗  RECOMMENDED RESOLUTION ACTIONS:''${NC}"
                echo -e "  ''${C_MUTED}────────────────────────────────────────────────────────────────────────''${NC}"
                
                if [ "''${RG_VAL}" == "✗" ]; then
                    echo -e "    ''${C_MUTED}•''${NC} ''${C_WHITE}ripgrep missing''${NC}  ''${C_MUTED}❯''${NC} Add ''${C_GOLD}\"ripgrep\"''${NC} to ''${C_HIGHLIGHT}core.nix''${NC} and run ''${C_PRIMARY}manx rebuild''${NC}"
                fi
                if [ "''${NH_VAL}" == "✗" ]; then
                    echo -e "    ''${C_MUTED}•''${NC} ''${C_WHITE}nh missing''${NC}       ''${C_MUTED}❯''${NC} Add ''${C_GOLD}\"nh\"''${NC} to ''${C_HIGHLIGHT}core.nix''${NC} and run ''${C_PRIMARY}manx rebuild''${NC}"
                fi
                if [ "''${FZF_VAL}" == "✗" ]; then
                    echo -e "    ''${C_MUTED}•''${NC} ''${C_WHITE}fzf missing''${NC}      ''${C_MUTED}❯''${NC} Add ''${C_GOLD}\"fzf\"''${NC} to ''${C_HIGHLIGHT}core.nix''${NC} and run ''${C_PRIMARY}manx rebuild''${NC}"
                fi
                if [ "''${GIT_VAL}" == "✗" ]; then
                    echo -e "    ''${C_MUTED}•''${NC} ''${C_WHITE}git missing''${NC}      ''${C_MUTED}❯''${NC} Add ''${C_GOLD}\"git\"''${NC} to ''${C_HIGHLIGHT}core.nix''${NC} and run ''${C_PRIMARY}manx rebuild''${NC}"
                fi
                if [ "''${OLLAMA_VAL}" == "✗" ]; then
                    echo -e "    ''${C_MUTED}•''${NC} ''${C_WHITE}Ollama offline''${NC}   ''${C_MUTED}❯''${NC} Run ''${C_PRIMARY}sudo systemctl start ollama''${NC} in terminal"
                fi
                if [ "''${TOKEN_VAL}" == "✗" ]; then
                    echo -e "    ''${C_MUTED}•''${NC} ''${C_WHITE}Token missing''${NC}    ''${C_MUTED}❯''${NC} Run ''${C_PRIMARY}manx aider''${NC} and paste your personal PAT"
                fi
            fi
