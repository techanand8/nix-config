...
function confirmDateSwitch(callback) {
  if (isDirty) {
    if (
      confirm(
        "Warning: You have unsaved changes on this page. Switching dates will discard your changes. Do you want to continue?",
      )
    ) {
      isDirty = false;
      callback();
...
function initApp() {
  ...
  inputReflection.addEventListener("input", (e) => {
    markDirty();
    document.getElementById("badgeReflection").innerText =
      `${e.target.value} / 10`;
  });
  ...
  if (inputSleep) {
    inputSleep.addEventListener("input", (e) => {
      markDirty();
      document.getElementById("badgeSleep").innerText =
        `${parseFloat(e.target.value).toFixed(1)} hrs`;
    });
  }
  ...
  if (inputScreen) {
    inputScreen.addEventListener("input", (e) => {
      markDirty();
      document.getElementById("badgeScreen").innerText =
        `${parseFloat(e.target.value).toFixed(1)} hrs`;
    });
  }
  ...
  document
    .getElementById("inputAcademicHours")
    .addEventListener("input", (e) => {
      markDirty();
      document.getElementById("valAcademicHours").innerText =
        e.target.value || 0;
    });
  ...
  document.querySelectorAll('input[name="vlsiType"]').forEach((radio) => {
    radio.addEventListener("change", (e) => {
      markDirty();
      document
        .querySelectorAll(".discipline-card")
  ...
  document
    .querySelectorAll('input[name="vlsiMethodologies"]')
    .forEach((box) => {
      box.addEventListener("change", markDirty);
    });
  ...
  document
    ...
    .addEventListener("click", resetFormValues);
...
function updateRoutineProgress() {
  ...
  document.getElementById("badgeRoutine").innerText =
    `${percentage}% Routine Match`;
  return percentage;
}
...
function populateForm(data) {
  ...
  document.getElementById("badgeFocus").innerText =
    `${data.focusScore || 0} / 10`;
  ...
  if (sliderSleep) {
    ...
    document.getElementById("badgeSleep").innerText =
      `${parseFloat(sleepHours).toFixed(1)} hrs`;
  }
  ...
  if (sliderScreen) {
    ...
    document.getElementById("badgeScreen").innerText =
      `${parseFloat(screenHours).toFixed(1)} hrs`;
  }

  // Text areas
  document.getElementById("inputYesterdayDone").value =
    data.yesterdayDone || "";
  document.getElementById("inputYesterdayImprove").value =
    data.yesterdayImprove || "";
  document.getElementById("inputTodayDone").value = data.todayDone || "";
  document.getElementById("inputTodayDoingNow").value =
    data.todayDoingNow || "";
  ...
  // Map 6 routine slots
  const routineTimeline = data.routineTimeline || [
    false,
    false,
    false,
    false,
...
function calculateDashboardMetrics() {
  ...
  const academicHoursSum = last7.reduce(
    (sum, item) => sum + (item.academicHours || 0),
    0,
  );
  document.getElementById("statTasksComp").innerText =
    `${academicHoursSum} hrs`;
...
async function saveCurrentData() {
  ...
  const academicFocus = [];
  document
    .querySelectorAll('input[name="academicFocus"]:checked')
    .forEach((box) => {
  ...
  const languages = [];
  document
    .querySelectorAll('input[name="vlsiLanguages"]:checked')
    .forEach((box) => {
  ...
  const methodologies = [];
  document
    .querySelectorAll('input[name="vlsiMethodologies"]:checked')
    .forEach((box) => {
  ...
  const payload = {
    ...
    academicHours: parseFloat(
      document.getElementById("inputAcademicHours").value || 0,
    ),
    // VLSI Section
  ...
  try {
    ...
    document.getElementById("saveStatus").innerText =
      "All data secure and locally synced.";
...
function resetFormValues() {
  if (
    confirm(
      "Are you sure you want to clear this entry? (Will reset values on screen back to clean defaults)",
    )
  ) {
    populateForm({
      date: currentState.date,
    ...
    showToast(
      "info",
      "Screen values reset. Press Save to write blank values to disk.",
    );
  }
}
...
function renderAnalyticsCharts() {
  ...
  const labels = last7.map((d) =>
    formatHumanDate(d.date).replace(", 2026", ""),
  );
